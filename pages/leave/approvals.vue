  ...
  <div class="bg-white p-4 rounded-lg shadow-md">
    <div class="mt-2">
      <div class="bg-emerald-600 text-white py-3 px-6 border-b flex items-center">
        <span class="text-sm font-bold">Leave Approval</span>
      </div>
      ...
      <div class="flex flex-wrap items-center justify-between mb-4 mt-4">
        ...
        <div class="flex space-x-4">
          ...
          <div class="flex flex-col">
            <USelect v-model="filter.duration" :items="durationOptions" class="w-48 mt-6" placeholder="Select Duration"></USelect>
          </div> -->
          <div class="flex flex-col">
            <label for="status" class="text-sm font-medium text-gray-700 mb-1">Status</label>
            <USelect v-model="filter.status" :items="statusOptions" class="w-48" placeholder="Select Status"></USelect>
          </div>
          <div class="flex flex-col">
              ...
              <td class="px-6 py-4 text-sm whitespace-nowrap">
                <div v-if="data.status === 'Pending'" class="flex space-x-2">
                  <UButton
                    ...
                    size="sm"
                    label="Approve"
                    @click="handleAction(data.id, 'Approved')"
                  />
                  <UButton
                    color="error"
                    variant="solid"
                    size="sm"
                    label="Reject"
                    @click="handleAction(data.id, 'Rejected')"
                  />
                </div>
                <span v-else>-</span>
              </td>
...
// Filter object with date filters and status (default to 'Pending')
const filter = ref({
  leave_type: null,
  duration: null,
...
// Function to get attachment icon class (using FontAwesome)
const getAttachmentIcon = (type) => {
  return type === 'pdf' ? 'fas fa-file-pdf' : 'fas fa-file-word';
};

// Filter Logic with Date Range and Status
const filteredData = computed(() => {
  return formData.value.filter(form => {
    ...
    return (
      ...
      (filter.value.status === null || form.status === filter.value.status) && // Added status filter
...
const handleAction = async (id, status) => {
  ...
  //     body: { status },
  //   });
  //   console.log(`Leave ${id} updated to ${status}:`, response);
    
  //   // Update local data
  //   const leave = formData.value.find(item => item.id === id);
  //   if (leave) {
  //     leave.status = status;
  //   }
  // } catch (error) {
